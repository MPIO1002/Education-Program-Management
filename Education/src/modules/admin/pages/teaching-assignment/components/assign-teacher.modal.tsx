...
const AssignTeacherModal: React.FC<AssignTeacherProps> = ({ onClose, onAssign }) => {
    ...
    useEffect(() => {
        ...
        setFormData(prev => ({...prev, ["vaiTro"]: "Phụ trách"}))
        setTimeout(() => setIsSubmit(false), 1000)
    }, [])

    useEffect(() => {
        var list = planGroups?.filter(item => {
            return !planGroupTeacher?.some(i => i === item.maNhom);
        });
        setPlanGroups(list);
    }, [planGroupTeacher])
    ...
    useEffect(() => {
        ...
        setFormData(prev => ({...prev, ["nhomId"]: planGroups[0].id}))
    }, [planGroups])

    useEffect(() => {
        if (!teachers || teachers.length == 0) return;
        setFormData(prev => ({...prev, ["giangVienId"]: teachers[0].id}))
    }, [teachers])
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/plan-group-teacher`, {
                method: 'POST',
                headers: {
            ...
            setNotification({ message: e.message || 'Failed to create. Please try again.', type: 'error' });
            setIsSubmit(false);
